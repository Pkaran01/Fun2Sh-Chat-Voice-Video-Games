...
public class DialogOccupantDataManager extends BaseManager<DialogOccupant> {
    ...
    public List<DialogOccupant> getDialogOccupantsListByDialogIdAndType(String dialogId,String dialogType) {
        List<DialogOccupant> dialogOccupantsList = null;

        try {
            QueryBuilder<DialogOccupant, Long> queryBuilder = dao.queryBuilder();
            queryBuilder.where().and(queryBuilder.where().eq(Dialog.Column.ID, dialogId),queryBuilder.where().eq(Dialog.Column.TYPE, dialogType));
            //where.and(where.eq(Account.NAME_FIELD_NAME, "foo"), where.eq(Account.PASSWORD_FIELD_NAME, "_secret"));
            PreparedQuery<DialogOccupant> preparedQuery = queryBuilder.prepare();
            dialogOccupantsList = dao.query(preparedQuery);
        } catch (SQLException e) {
